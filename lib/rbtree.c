...
void debug_rb_tree(struct rb_node *node)
{
	get_outermost(node->child[RB_LEFT], RB_LEFT);
	get_outermost(node->child[RB_LEFT], RB_RIGHT);
	get_outermost(node->child[RB_RIGHT], RB_LEFT);
	get_outermost(node->child[RB_RIGHT], RB_RIGHT);
}
