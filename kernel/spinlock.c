...
void __spin_lock(struct spinlock *lock, const char *file, int line)
{
	...
	if (cur_task)
		debug_print("(CPU: %d | PID: %3d)$ Lock acquired!\n", this_cpu->cpu_id, cur_task->task_pid);
	else
		debug_print("(CPU: %d | PID: %3s)$ Lock acquired!\n", this_cpu->cpu_id, "N/A");
...
void __spin_unlock(struct spinlock *lock, const char *file, int line)
{
	...
	atomic_barrier();
	if (cur_task)
		debug_print("(CPU: %d | PID: %3d)$ Lock released.\n", this_cpu->cpu_id, cur_task->task_pid);
	else
		debug_print("(CPU: %d | PID: %3s)$ Lock released.\n", this_cpu->cpu_id, "N/A");
	lock->locked = 0;
	atomic_barrier();
}
