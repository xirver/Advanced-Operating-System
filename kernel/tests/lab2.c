...
int lab2_check_pte_vas_ext(physaddr_t *entry, uintptr_t base, uintptr_t end,
    struct page_walker *walker)
{
	(void)walker;
...
int lab2_check_pde_vas_ext(physaddr_t *entry, uintptr_t base, uintptr_t end,
    struct page_walker *walker)
{
...
void lab2_check_vas_ext(void)
{
	struct page_walker walker = {
		.pte_callback = lab2_check_pte_vas_ext,
		.pde_callback = lab2_check_pde_vas_ext,
	};
...
void lab2_check_buddy_full_consistency(void)
{
	struct page_info *page;
	physaddr_t addr;
	...
	for (addr = 0;
		...
		if (!page_lookup(kernel_pml4, pa2page(addr), NULL)) {
                ...
                // check if the physical page is mapped in the physmap
...
#ifdef EXTENDED_CHECKS_LAB2
        lab2_check_buddy_full_consistency();
#endif
	lab2_check_vas();
